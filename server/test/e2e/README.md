...
Both can be created by running `sudo sh test/e2e/install.sh` from within your cliknews directory. This creates a MYSQL user and database called `cliknews_test`, and generates the required `config/test.toml`.
...
1. Start ClikNews with `npm run starttest`
...
* `npm run sqldumptest` - exports the test DB to `setup/sql/cliknews-test.sql`
* `npm run sqlresettest` - drops all tables then loads `setup/sql/cliknews-test.sql`
