...
router.getAsync('/:campaign/:list/:subscription/:link', async (req, res) => {
    ...
    if (link) {
        // In Cliker v1 we would do the URL expansion here based on merge tags. We don't do it here anymore. Instead, the URLs are expanded when message is sent out (in links.updateLinks)
