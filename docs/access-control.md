...
and low-level, intended to be configured once through the Cliker config file. The high-level
layer serves for providing access to variuous resources, while the low-level layer is meant
to define the access roles in Cliker to reflect an organisational or process hierarchy.

### High-level access management (through web UI)

On the high abstraction level, which is accessible to users via the web-based UI, Cliker
...
Cliker further features hierarchical namespaces. Every entity has to reside in a namespace 
...
Cliker resets these default shares at start and also whenever permission cache is rebuilt
...
Internally, Cliker relies on fine-grained permissions, which are triplets: 
...
Cliker's config file defines the roles (available in the high-level access management) and 
...
more enterprise applications where a single installation of Cliker serves a number of rather
independent groups.
