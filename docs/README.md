...
[Cliker](https://github.com/rbassoi/cliker) is a self hosted newsletter application built on Node.js (v10+) and MySQL (v8+) or MariaDB (v10+).
...
Cliker supports subscriber list management, list segmentation, custom fields, email templates, large CSV list import files, etc.

Subscribe to Cliker Newsletter [here](http://cliker.org/subscription/EysIv8sAx) (uses Cliker obviously)
...
![](https://cliker.org/images/iredmail-logo-red-114.png)
...
## Cons
  ...
  * Beta-grade software. Might or might not work as expected. There are several users with list sizes between 100k and 1M and Cliker seems to work for them but YMMV
  * Almost no documentation (there are some guides in the [Wiki](https://github.com/rbassoi/cliker/wiki))
...
Cliker and all required dependencies (including MySQL). The installation script assumes a somewhat blank server, so if this is a machine you are already using for something else, you might want to skip the automatic install and proceed manually.
...
```
curl https://raw.githubusercontent.com/rbassoi/cliker/master/setup/install.sh | sudo bash
```
...
Install script installs and sets up the following:
  ...
  * **Cliker** (from the master branch) on port 80
  ...
  * **logrotate** to rotate Cliker log files
  * **upstart** or **systemd** init script to automatically start and manage Cliker process

After the install script has finished and you see a success message then you should have a Cliker instance running at http://yourdomain.com
...
If you are using the bundled ZoneMTA then you should make sure you are using a proper PTR record for your server. For example if you use DigitalOcean then PTR is set automatically (it's the droplet name, so make sure your droplet name is the same as the domain name you are running Cliker from). If you use AWS then you can request setting up PTR records using [this form](https://portal.aws.amazon.com/gp/aws/html-forms-controller/contactus/ec2-email-limit-rdns-request) (requires authentication). Otherwise you would have to check from your service provider, hot to get the PTR record changed. Everything should work without the PTR record but setting it up correctly improves the deliverability a lot.
...
DKIM, DMARK, SPF and PTR are DNS records which spam filters use to figure out if e-mails were really sent by you (and not by a spammer who tries to conceal his identity to be able to continue send bulks of e-mails people never subscribed for). Assuming that you use zone-mta and your e-mails are to originate from a Cliker installation at `cliker.example.com` and optionally from `mail.example.net`, to practically set all these records up you will need to:
...
pushd /opt/dkim-keys
openssl genrsa -out cliker.example.com.key 2048 # private key cliker.example.com.key
openssl rsa -in cliker.example.com.key -out cliker.example.com.pub -pubout -outform PEM # public key cliker.example.com.pub
```

2. add 3 new txt records for the cliker.example.com that will most likely similar to the example below:

```
default._domainkey.cliker.example.com     TXT    "k=rsa; p=[public key in one line];"
cliker.example.com                        TXT    "v=spf1 mx a a:mail.example.net -all"
_dmarc.cliker.example.com                 TXT    "v=DMARC1; p=reject"
```

(refer to a google search for a DKIM generator, SPF generator and DMARC genreator to get you up to speed). Configure your Cliker settings accoring to this:

**DKIM domain:** cliker.example.com
**DKIM selector:** default
**DKIM Private Key:** [copy and paste the private key in /opt/dkim-keys/cliker.example.com.key]

The above steps will have the following effect:

- all messages sent by Cliker / Zone-mta will be signed by the DKIM Private Key (the signature becomes a part of the e-mail)
...
- additionally, the spamfilter will look for a TXT SPF record and will look a if the e-mail was sent from the IP address of cliker.example.com or mail.example.net.  If the sender IP or domain is different, it will discard the e-mail as spam.
...
```
10.27/1.110.220.in-addr.arpa.   1800    PTR     cliker.example.com.
```

If you run Cliker on a VPS, you will have to find the PTR configuration somewhere in your administration interface or ask your provider to help you.
...
* Download Cliker files using git: `git clone git://github.com/rbassoi/cliker.git` (or download [zipped repo](https://github.com/rbassoi/cliker/archive/master.zip)) and open Cliker folder `cd cliker`
...
* If you want to use only / copy the `docker-compose.yml` file (for example, if you were deploying with Rancher), you may need to first run `docker-compose build` to make sure your system has a Docker image `cliker:latest`.
...
### Manual Install (any OS that supports Node.js)
  ...
  1. Download Cliker files using git: `git clone git://github.com/rbassoi/cliker.git` (or download [zipped repo](https://github.com/rbassoi/cliker/archive/master.zip)) and open Cliker folder `cd cliker`
  2. Run `npm install --production` in the Cliker folder to install required dependencies
...
## Upgrade
  ...
  * Replace old files with new ones by running in the Cliker folder `git pull origin master` if you used Git to set Cliker up or just download [new files](https://github.com/rbassoi/cliker/archive/master.zip) and replace old ones with these
  * Run `npm install --production` in the Cliker folder
...
Cliker uses [node-config](https://github.com/lorenwest/node-config) for configuration management and thus the config files are loaded in the following order:
...
Edit [cliker.nginx](setup/cliker-nginx.conf) (update `server_name` directive) and copy it to `/etc/nginx/sites-enabled`

### Running as an Upstart Service in Ubuntu 14.04

Edit [cliker.conf](setup/cliker.conf) (update application folder) and copy it to `/etc/init`

## Subscription Widget

The (experimental) Cliker Subscription Widget allows you to embed your sign-up forms on your website. To embed a Widget, you need to:
...
```
<div data-cliker-subscription-widget data-url="http://lists.example.com/subscription/Byf44R-og/widget">
...
You can easily install and self-host Cliker on the Cloudron to send newsletters from your custom domain:

[![Install](https://cloudron.io/img/button.svg)](https://cloudron.io/button.html?app=org.cliker.cloudronapp)

The source code for the Cloudron app is [here](https://git.cloudron.io/cloudron/cliker-app).

## Bounce Handling

Cliker uses webhooks integration to detect bounces and spam complaints. Currently supported webhooks are:
  ...
  * **SparkPost** – use `http://domain/webhooks/sparkpost` as the webhook URL for bounces and complaints ([instructions](https://github.com/rbassoi/cliker/wiki/Setting-up-Webhooks-for-SparkPost))
  * **SendGrid** – use `http://domain/webhooks/sendgrid` as the webhook URL for bounces and complaints ([instructions](https://github.com/rbassoi/cliker/wiki/Setting-up-Webhooks-for-SendGrid))
  * **Mailgun** – use `http://domain/webhooks/mailgun` as the webhook URL for bounces and complaints ([instructions](https://github.com/rbassoi/cliker/wiki/Setting-up-Webhooks-for-Mailgun))
  * **ZoneMTA** – use `http://domain/webhooks/zone-mta` as the webhook URL for bounces. If you install Cliker with the included installation script then this route gets set up automatically during the installation process
  * **Postfix** – This is not a webhook but a TCP server on port 5699 to listen for piped Postfix logs. Enable it with the `[postfixBounce]` config option. To use it, pipe the log to that port using *tail*: `tail -F /var/log/mail.log | nc localhost 5699 -` (if Cliker restarts then you need to re-establish the *tail* pipe), alternatively you could send the log with a cron job periodically `tail -n 100 | nc localhost 5699 -`.

Additionally Cliker (v1.1+) is able to use VERP-based bounce handling. This would require to have a compatible SMTP relay (the services mentioned above strip out or block VERP addresses in the SMTP envelope) and you also need to set up special MX DNS name that points to your Cliker installation server.
...
There is a built in /dev/null server in Cliker that you can use to load test your installation. Check the `[testServer]` section in the configuration file for details. By default the test server is disabled. The server uses only cleartext connections, so select "Do not use encryption" in the encryption settings when setting up the server data in Cliker.
...
Cliker is currently not translated but it supports translations. To add translations you first need to add translation support for the translatable strings. To test if strings are translatable or not, use a fake language with code "zz"
...
let str2 = util.format( _('My name is "%s"'), 'Cliker');
```
...
```handlebars
<p>
    Cliker – the best newsletter app
</p>
```
...
* To create the translation catalog run `grunt` from command line. This fetches all translatable strings from JavaScript and Handlebars files and merges these into the translation catalog located at */languages/cliker.pot*

* To add a new language use this catalog file as source. Once you want to update your translation file from the updated catalog, then select "Catalogue" -> "Update from POT file..." in POEdit and select cliker.pot. This would merge all new translations from the POT file to your PO file.
...
* Once you have a correct MO file in the languages folder, then edit Cliker config and set ["language" option](https://github.com/rbassoi/cliker/blob/ba8bd1212335cb9bd7ba094beb7b5400f35cae6c/config/default.toml#L30-L31) to your language name. If the value is "et" then Cliker loads translations from ./languages/et.mo

> **NB!** For now translation settings are global, so if you have set a translation in config then this applies to all users. An user can't select another translation than the default even if there is a translation file. This is because current Cliker code does not provide request context to functions and the functions generating strings do not know which language to use.
