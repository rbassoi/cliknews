            ...
            <div>
                ...
                <Form stateOwner={this} onSubmitAsync={::this.submitHandler}>
                    ...
                    <Fieldset label={t('verpBounceHandling')}>
                        <Trans i18nKey="clikerIsAbleToUseVerpBasedRoutingTo"><p>Cliker is able to use VERP based routing to detect bounces. In this case the message is sent to the recipient using a custom VERP address as the return path of the message. If the message is not accepted a bounce email is sent to this special VERP address and thus a bounce is detected.</p></Trans>
                        <Trans i18nKey="toGetVerpWorkingYouNeedToSetUpADnsMx"><p>To get VERP working you need to set up a DNS MX record that points to your Cliker hostname. You must also ensure that Cliker VERP interface is available from port 25 of your server (port 25 usually requires root user privileges). This way if anyone tries to send email to someuser@verp-hostname then the email should end up to this server.</p></Trans>
