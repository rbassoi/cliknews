...
export function getMailerTypes(t) {
    ...
    if (clikerConfig.builtinZoneMTAEnabled) {
        zoneMtaTypeOptions.push({ key: ZoneMTAType.BUILTIN, label: t('builtinZoneMta')});
    }
    zoneMtaTypeOptions.push({ key: ZoneMTAType.WITH_CLIKER_HEADER_CONF, label: t('dynamicConfigurationOfDkimKeysViaZoneMt')});
    ...
    mailerTypes[MailerType.ZONE_MTA] = {
            ...
            return (
                <div>
                    <Fieldset label={t('mailerSettings')}>
                        ...
                        {(zoneMtaType === ZoneMTAType.REGULAR || zoneMtaType === ZoneMTAType.WITH_CLIKER_HEADER_CONF || zoneMtaType === ZoneMTAType.WITH_HTTP_CONF) &&
                            <div>
                                <InputField id="smtpHostname" label={t('hostname')} placeholder={t('hostnameEgSmtpexamplecom')}/>
                                <InputField id="smtpPort" label={t('port')} placeholder={t('portEg465AutodetectedIfLeftBlank')}/>
                                <Dropdown id="smtpEncryption" label={t('encryption')} options={smtpEncryptionOptions}/>
                    ...
                    {(zoneMtaType === ZoneMTAType.BUILTIN || zoneMtaType === ZoneMTAType.WITH_CLIKER_HEADER_CONF || zoneMtaType === ZoneMTAType.WITH_HTTP_CONF) &&
                        <Fieldset label={t('dkimSigning')}>
                            <Trans i18nKey="ifYouAreUsingZoneMtaThenClikerCan"><p>If you are using ZoneMTA then Cliker can provide a DKIM key for signing all outgoing messages.</p></Trans>
        ...
        initData: () => ({
            ...getInitGenericSMTP(),
            zoneMtaType: clikerConfig.builtinZoneMTAEnabled ? ZoneMTAType.BUILTIN : ZoneMTAType.REGULAR,
            dkimApiKey: '',
            dkimDomain: '',
            dkimSelector: '',
            dkimPrivateKey: ''
        ...
        beforeSave: data => {
            ...
            if (zoneMtaType === ZoneMTAType.BUILTIN || zoneMtaType === ZoneMTAType.WITH_HTTP_CONF || zoneMtaType === ZoneMTAType.WITH_CLIKER_HEADER_CONF) {
