...
MYSQL_DATABASE=${MYSQL_DATABASE:-'cliker'}
MYSQL_USER=${MYSQL_USER:-'cliker'}
MYSQL_PASSWORD=${MYSQL_PASSWORD:-'cliker'}
...
# Warning for users that already rely on the CLIKER_SETTING variable
# Can probably be removed in the future.
CLIKER_SETTING=${CLIKER_SETTINGS:-}
if [ ! -z "$CLIKER_SETTING" ]; then
    echo 'Error: CLIKER_SETTINGS is no longer supported. See README.md'
    exit 1
fi
...
postfixBounce:
  # Enable to allow writing Postfix bounce log to Cliker listener
  # If enabled, tail mail.log to Cliker with the following command:
